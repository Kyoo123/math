...
function calculatePolarToRect() {
    ...
    const real = (magnitude * Math.cos(angle * Math.PI / 180)).toFixed(2);
    const imaginary = (magnitude * Math.sin(angle * Math.PI / 180)).toFixed(2);

    const results = `
        <h3>Polar to Rectangular</h3>
        Real Part: ${real} <br>
        Imaginary Part: ${imaginary} <br>
        Rectangular Form: ${real} + ${imaginary}i
    `;

    document.getElementById("results").innerHTML = results;
    plotComplexNumber(parseFloat(real), parseFloat(imaginary));
}
